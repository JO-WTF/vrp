...
如果你已经在仓库根目录成功运行过上层 `build.sh` / `build.ps1`，并希望一次性完成 VRP Studio 后端依赖安装、前端构建，并将前端静态资源部署到后端包中，可以在仓库根目录执行：
...
3. 安装 `vrp-studio` 后端包本身（假设上层 `build.sh` / `build.ps1` 已经构建并安装好本地 `vrp-cli` Python 绑定）。
